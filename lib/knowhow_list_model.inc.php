...
abstract class knowhow_list_model
{
    /* Public properties */
    public $num_rows = 0;
    public $num2show = 10;
    ...
    protected $searchable_fields = array('title', 'author', 'subject', 'tag', 'number',
	'notes', 'main_text', 'en_text', 'publisher', 'create_date', 'revision_date',
	'reference');
	...
	$_authors = '';
	$_sql_str = 'SELECT a.author_name FROM knowhow_author AS ba
            LEFT JOIN knowhow AS b ON ba.biblio_id=b.biblio_id
    ...
    protected function makeOutput()
    {
        ...
        while ($_biblio_d = $this->resultset->fetch_assoc()) {
            $_biblio_d['title'] = '<a href="'.$sysconf['baseurl'].'index.php?p=show_kh_detail&id='.$_biblio_d['biblio_id'].'" class="titleField" title="'.__('Record Detail').'">'.$_biblio_d['title'].'</a>';
            // label
            ...
            // button
            $_biblio_d['detail_button'] = '<a href="'.$sysconf['baseurl'].'index.php?p=show_kh_detail&id='.$_biblio_d['biblio_id'].'" class="detailLink" title="'.__('Record Detail').'">'.__('Record Detail').'</a>';
            if ($this->xml_detail) {
                $_biblio_d['xml_button'] = '<a href="'.$sysconf['baseurl'].'index.php?p=show_kh_detail&inXML=true&id='.$_biblio_d['biblio_id'].'" class="xmlDetailLink" title="View Detail in XML Format" target="_blank">XML Detail</a>';
            } else {
			...
			if(isset($_biblio_d['number']) AND !(empty($_biblio_d['number']))) {
                $_buffer .= '<div class="subItem authorField"><b>'.__('Number').'</b> : '.$_biblio_d['number'].'</div>';
			}
    ...
    public function XMLresult()
    {
        ...
        while ($_biblio_d = $this->resultset->fetch_assoc()) {
            ...
            $_biblio_authors_q = $this->obj_db->query('SELECT a.*,ba.level FROM mst_author AS a'
                .' LEFT JOIN knowhow_author AS ba ON a.author_id=ba.author_id WHERE ba.biblio_id='.$_biblio_d['biblio_id']);
