...
if (isset($_POST['upload']) AND trim(strip_tags($_POST['fileTitle'])) != '') {
    ...
    if (isset($_FILES['file2attach']) AND $_FILES['file2attach']['size']) {
        ...
        if ($file_upload_status === UPLOAD_SUCCESS) {
            ...
            @$sql_op->insert('knowhow_files', $fdata);
    ...
    if (isset($_POST['updateBiblioID']) AND !empty($_POST['updateBiblioID'])) {
        ...
        if (isset($_POST['updateFileID'])) {
            ...
            $update1 = $sql_op->update('knowhow_attachment', array('access_type' => $data['access_type'], 'access_limit' => $data['access_limit']), 'biblio_id='.$updateBiblioID.' AND file_id='.$fileID);
            // file description update
            $update2 = $sql_op->update('knowhow_files', array('file_title' => $title, 'file_url' => $url, 'file_desc' => $dbs->escape_string(trim($_POST['fileDesc']))), 'file_id='.$fileID);
            if ($update1) {
                echo '<script type="text/javascript">';
                echo 'alert(\''.__('File Attachment data updated!').'\');';
                echo 'parent.setIframeContent(\'attachIframe\', \''.MODULES_WEB_ROOT_DIR.'knowhow/iframe_attach.php?biblioID='.$updateBiblioID.'\');';
                echo '</script>';
            } else {
                utility::jsAlert(''.__('File Attachment data FAILED to update!').''."\n".$sql_op->error);
            }
        } else {
            if ($sql_op->insert('knowhow_attachment', $data)) {
                echo '<script type="text/javascript">';
                echo 'alert(\''.__('File Attachment uploaded succesfully!').'\');';
                echo 'parent.setIframeContent(\'attachIframe\', \''.MODULES_WEB_ROOT_DIR.'knowhow/iframe_attach.php?biblioID='.$data['biblio_id'].'\');';
                echo '</script>';
            } else {
        ...
        if ($uploaded_file_id) {
            ...
            echo '<script type="text/javascript">';
            echo 'alert(\''.__('File Attachment uploaded succesfully!').'\');';
            echo 'parent.setIframeContent(\'attachIframe\', \''.MODULES_WEB_ROOT_DIR.'knowhow/iframe_attach.php\');';
            echo '</script>';
        }
    }
}
...
// query
$file_attach_q = $dbs->query("SELECT fl.*, batt.* FROM knowhow_files AS fl
    LEFT JOIN knowhow_attachment AS batt ON fl.file_id=batt.file_id
