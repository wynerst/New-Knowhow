...
if (isset($_POST['save']) AND (isset($_POST['topicID']) OR trim($_POST['search_str']))) {
    ...
    if (isset($_POST['biblioID']) AND !empty($_POST['biblioID'])) {
        ...
        if ($sql_op->insert('knowhow_topic', $data)) {
            echo '<script type="text/javascript">';
            echo 'alert(\'Topic succesfully updated!\');';
            echo 'parent.setIframeContent(\'topicIframe\', \''.MODULES_WEB_ROOT_DIR.'knowhow/iframe_topic.php?biblioID='.$data['biblio_id'].'\');';
            echo '</script>';
        } else {
        ...
        echo 'parent.setIframeContent(\'topicIframe\', \''.MODULES_WEB_ROOT_DIR.'knowhow/iframe_topic.php\');';
        echo '</script>';
    }
}
