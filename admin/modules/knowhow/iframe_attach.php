...
if (isset($_POST['bid']) AND isset($_POST['remove'])) {
    ...
    $file_q = $dbs->query('SELECT * FROM knowhow_files WHERE file_id='.$file);
    $file_d = $file_q->fetch_assoc();
    // attachment data delete
    $sql_op = new simbio_dbop($dbs);
    $sql_op->delete('knowhow_attachment', "file_id=$file AND biblio_id=$bid");
...
if ($biblioID) {
    ...
    // database list
    $biblio_attach_q = $dbs->query('SELECT att.*,fl.* FROM knowhow_attachment AS att
        LEFT JOIN knowhow_files AS fl ON att.file_id=fl.file_id WHERE biblio_id='.$biblioID);
    ...
    while ($biblio_attach_d = $biblio_attach_q->fetch_assoc()) {
        ...
        // edit link
        $edit_link = '<a href="#" onclick="top.openHTMLpop(\''.MODULES_WEB_ROOT_DIR.'knowhow/pop_attach.php?biblioID='.$biblioID.'&fileID='.$biblio_attach_d['file_id'].'\', 600, 300, \''.__('File Attachments').'\')">Edit</a>';
