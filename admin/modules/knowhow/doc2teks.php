...
/* Read text from DOC MsWord file */
...
if (isset($_GET['fid']) AND isset($_GET['bid']) AND $can_read AND $can_write) {
    ...
    if (empty($bib_id) AND empty($file_id)) {
		...
		if (empty($extraction)) {
			...
			$form->addTextField('textarea', 'extract', __('DOC MsWord extraction result').'*', $extraction, 'rows="10" style="width: 100%; overflow: auto;"');
			// add /replace option
    ...
    exit();
		...
		if ($sql_op->update('knowhow', $data, 'biblio_id ='.$biblio_id)) {
			utility::jsAlert(__('DOC MsWord text extracted and add to KH data successfully'));
...
<div class="menuBoxInner masterFileIcon">
    <?php echo strtoupper(__('DOC MsWord Text Extractions')); ?> - <a href="<?php echo MODULES_WEB_ROOT_DIR; ?>knowhow/doc2teks.php" class="headerText2"><?php echo __('List DOC MsWord file available'); ?></a>
    <hr />
    <form name="search" action="<?php echo MODULES_WEB_ROOT_DIR; ?>knowhow/doc2teks.php" id="search" method="get" style="display: inline;"><?php echo __('Search'); ?> :
...
if ($can_read AND $can_write) {
	...
	'concat("<a href=""../admin/modules/knowhow/doc2teks.php?fid=", att.file_id, "&amp;bid=", att.biblio_id, """>Add text</a>") AS Extract');
} else {
...
if (isset($_GET['keywords']) AND $_GET['keywords']) {
   ...
   $datagrid->setSQLCriteria("f.file_name LIKE '%$keywords%' OR 
		...
		k.kh_type LIKE '%$keywords%' AND f.mime_type='application/msword'");
} else {
   $datagrid->setSQLCriteria("f.mime_type='application/msword'");	
}
