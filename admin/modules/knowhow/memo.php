    ...
    if (empty($title)) {
        ...
        if (isset($_POST['updateRecordID'])) {
            ...
            if ($insert) {
                ...
                echo '<script type="text/javascript">parent.$(\'#mainContent\').simbioAJAX(\''.MODULES_WEB_ROOT_DIR.'knowhow/memo.php\', {method: \'post\', addData: \'itemID='.$last_biblio_id.'&detail=true\'});</script>';
...
<div class="menuBoxInner biblioIcon">
    <?php echo strtoupper(__('Bibliographic')); ?> - <a href="<?php echo MODULES_WEB_ROOT_DIR; ?>knowhow/memo.php?action=detail" class="headerText2"><?php echo __('Add New Memo'); ?></a>
    &nbsp; <a href="<?php echo MODULES_WEB_ROOT_DIR; ?>knowhow/memo.php" class="headerText2"><?php echo __('Memo List'); ?></a>
    <?php
    // enable UCS?
    if ($sysconf['ucs']['enable']) {
    ?>
    ...
    ?>
    <hr />
    <form name="search" action="<?php echo MODULES_WEB_ROOT_DIR; ?>knowhow/memo.php" id="search" method="get" style="display: inline;"><?php echo __('Search'); ?> :
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
    ...
    // created date
    $form->addDateField('create_date', __('Created Date'), $rec_d['create_date']?$rec_d['create_date']:date('Y-m-d'));
...
//            $datagrid->modifyColumnContent(1, 'callback{showTitleAuthors}');
    ...
    if (isset($criteria)) {
        $datagrid->setSQLcriteria('('.$criteria['sql_criteria'].') AND (kh_type=\'memo\')');
    } else {
        $datagrid->setSQLcriteria('(kh_type=\'memo\')');
	}
