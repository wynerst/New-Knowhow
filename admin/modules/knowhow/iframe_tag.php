...
if (isset($_GET['removesess'])) {
    $idx = $_GET['removesess'];
    unset($_SESSION['biblioTag'][$idx]);
    echo '<script type="text/javascript">';
    echo 'alert(\'Topic removed!\');';
    echo 'location.href = \'iframe_tag.php\';';
    echo '</script>';
}
...
if (isset($_POST['remove'])) {
    ...
    $sql_op->delete('knowhow_tag', 'topic_id='.$id.' AND biblio_id='.$bid);
    echo '<script type="text/javascript">';
    echo 'alert(\'Topic succesfully removed!\');';
    echo 'location.href = \'iframe_tag.php?biblioID='.$bid.'\';';
    echo '</script>';
}
...
if ($biblioID) {
    ...
    // database list
    $biblio_topic_q = $dbs->query("SELECT kt.*, t.topic FROM knowhow_tag AS kt
        LEFT JOIN mst_tag AS t ON kt.topic_id=t.topic_id
        WHERE kt.biblio_id=$biblioID ORDER BY t.last_update ASC");
...
//        $table->appendTableRow(array($remove_link, $topic, $sysconf['subject_level'][$biblio_topic_d['level']]));
    ...
    if ($_SESSION['biblioTag']) {
        ...
        foreach ($_SESSION['biblioTag'] as $biblio_session) {
            // remove link
            $remove_link = '<a href="iframe_tag.php?removesess='.$biblio_session[0].'"
                style="color: #000000; text-decoration: underline;">Remove</a>';

            if ($biblio_session) {
                $topic_q = $dbs->query("SELECT topic FROM mst_tag WHERE topic_id=".$biblio_session[0]);
