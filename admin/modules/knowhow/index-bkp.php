        ...
        if (isset($_POST['updateRecordID'])) {
            ...
            if ($insert) {
                ...
                if ($_SESSION['biblioTag']) {
                    foreach ($_SESSION['biblioTag'] as $attachment) {
                        $sql_op->insert('knowhow_tag', array('biblio_id' => $last_biblio_id, 'topic_id' => $topic['0'], 'level' => $topic[1]));
                    }
                }
...
<div class="menuBoxInner biblioIcon">
    <?php echo strtoupper(__('Bibliographic')); ?> - <a href="<?php echo MODULES_WEB_ROOT_DIR; ?>knowhow/index.php?action=detail" class="headerText2"><?php echo __('Add New KnowHow'); ?></a>
    &nbsp; <a href="<?php echo MODULES_WEB_ROOT_DIR; ?>knowhow/index.php" class="headerText2"><?php echo __('KH Bibliographic List'); ?></a>
    <?php
    // enable UCS?
    if ($sysconf['ucs']['enable']) {
    ?>
    ...
    <select name="field"><option value="0"><?php echo __('All Fields'); ?></option><option value="title"><?php echo __('Title/Series Title'); ?> </option><option value="subject"><?php echo __('Topics'); ?></option><option value="author"><?php echo __('Authors'); ?></option><option value="isbn"><?php echo __('Number'); ?></option><option value="publisher"><?php echo __('Publisher'); ?></option></select>
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
    ...
    // regulation number
    $form->addTextField('text', 'number', __('Number'), $rec_d['nummber'], 'style="width: 100%;"');
    // biblio specific detail info/area
    $form->addTextField('text', 'publisher', __('Publisher'), $rec_d['publisher'], 'style="width: 100%"');
    // regulation number
    $form->addTextField('text', 'create_date', __('Date of Issue'), $rec_d['create_date'], 'style="width: 40%;"');
    ...
    // knowhow_tag
        ...
        $str_input .= '<iframe name="tagIframe" id="topicIframe" class="borderAll" style="width: 100%; height: 70px;" src="'.MODULES_WEB_ROOT_DIR.'knowhow/iframe_tag.php?biblioID='.$rec_d['biblio_id'].'&block=1"></iframe>';
    $form->addAnything(__('Subject(s)'), $str_input);
    // biblio language
    ...
    // tag
    $form->addTextField('textarea', 'tag', __('Tag'), $rec_d['tag'], 'style="width: 100%;" rows="2"');
    ...
    if ($sysconf['index']['type'] == 'index' ||  $sysconf['index']['type'] == 'sphinx' ) {
        ...
        // table spec
        $table_spec = 'knowhow LEFT JOIN item ON knowhow.biblio_id=item.biblio_id';
    ...
    if (isset($_GET['keywords']) AND $_GET['keywords']) {
        ...
        $searchable_fields = array('title', 'author', 'subject', 'publisher');
