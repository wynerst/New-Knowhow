...
{
    global $dbs;
    $_q = $dbs->query('SELECT topic_id FROM mst_tag WHERE topic=\''.$str_subject.'\'');
    if ($_q->num_rows > 0) {
        $_d = $_q->fetch_row();
...
if (isset($_POST['save']) AND (isset($_POST['topicID']) OR trim($_POST['search_str']))) {
    ...
    if (isset($_POST['biblioID']) AND !empty($_POST['biblioID'])) {
        ...
        if ($sql_op->insert('knowhow_tag', $data)) {
            echo '<script type="text/javascript">';
            echo 'alert(\'Topic succesfully updated!\');';
            echo 'parent.setIframeContent(\'tagIframe\', \''.MODULES_WEB_ROOT_DIR.'knowhow/iframe_tag.php?biblioID='.$data['biblio_id'].'\');';
            echo '</script>';
        } else {
...
<form name="mainForm" action="pop_tag.php?biblioID=<?php echo $biblioID; ?>" method="post">
<div>
    <strong><?php echo __('Add Subject'); ?></strong>
    <hr />
    <form name="searchTopic" method="post" style="display: inline;">
    <?php
    $ajax_exp = "ajaxFillSelect('../../AJAX_lookup_handler.php', 'mst_tag', 'topic_id:topic', 'topicID', $('#search_str').val())";
    ?>
