...
if (isset($_POST['remove'])) {
    ...
    $sql_op->delete('knowhow_topic', 'topic_id='.$id.' AND biblio_id='.$bid);
    echo '<script type="text/javascript">';
...
if ($biblioID) {
    ...
    // database list
    $biblio_topic_q = $dbs->query("SELECT bt.*, t.topic FROM knowhow_topic AS bt
        LEFT JOIN mst_topic AS t ON bt.topic_id=t.topic_id
        WHERE bt.biblio_id=$biblioID ORDER BY bt.level ASC");
